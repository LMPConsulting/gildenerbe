// Baut aus sperre/src/* eine einzige, in sich geschlossene HTML-Datei.
// Kein CDN, keine externen Dateien: läuft per Doppelklick (file://) und offline.
//
//   node sperre/build.mjs            -> sperre/index.html
//   node sperre/build.mjs --fragment out.html   (Rumpf ohne <html>/<head>, für Artifacts)
...
const MODULE = ['src/qr.js', 'src/funk.js', 'src/netz.js',
  'src/brett.js', 'src/engine.js', 'src/ui.js'];
...
// Ein weißer Sperrstein zwischen zwei Figuren, auf hellem Karton.
const ICON = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">`
  + `<rect width="64" height="64" rx="14" fill="#171d2b"/>`
  + `<rect x="8" y="8" width="48" height="48" rx="10" fill="#eee9df"/>`
  + `<path d="M22 44 V30 a10 10 0 0 1 20 0 v14 z" fill="#fdfcf8" stroke="#9a927f" stroke-width="2"/>`
  + `<circle cx="15" cy="20" r="5.5" fill="#d99327"/>`
  + `<circle cx="49" cy="20" r="5.5" fill="#2f9c95"/></svg>`;
const iconUrl = 'data:image/svg+xml,' + encodeURIComponent(ICON);

const manifest = {
  name: 'Sperrsteine', short_name: 'Sperren', start_url: '.', display: 'standalone',
  orientation: 'portrait', background_color: '#171d2b', theme_color: '#171d2b',
  description: 'Sperrsteine — fünf Figuren, elf Steine im Weg, die erste Figur oben gewinnt. Komplett offline.',
...
<meta name="description" content="Sperrsteine – Wegespiel mit Sperren für zwei. An einem Handy oder auf zweien, komplett offline.">
<title>Sperrsteine</title>
...
if (webFlagge !== -1) {
  ...
  const mitnahme = 'Sperrsteine.html';
...
if (fragmentFlag !== -1) {
  const out = resolve(process.argv[fragmentFlag + 1] || 'sperre-fragment.html');
  writeFileSync(out, `<title>Sperrsteine</title>\n<style id="sperre-css">\n${css}\n</style>\n${body}\n`);
