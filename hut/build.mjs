// Baut aus hut/src/* eine einzige, in sich geschlossene HTML-Datei.
// Kein CDN, keine externen Dateien: läuft per Doppelklick (file://) und offline.
//
//   node hut/build.mjs            -> hut/index.html
//   node hut/build.mjs --fragment out.html   (Rumpf ohne <html>/<head>, für Artifacts)
...
// Zwei Hütchen auf hellem Karton.
const ICON = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">`
  + `<rect width="64" height="64" rx="14" fill="#16281f"/>`
  + `<rect x="8" y="8" width="48" height="48" rx="10" fill="#f2e9d8"/>`
  + `<path d="M23 17 L31 37 Q23 41 15 37 Z" fill="#d29a1e"/>`
  + `<path d="M42 26 L50 46 Q42 50 34 46 Z" fill="#7d4fae"/></svg>`;
const iconUrl = 'data:image/svg+xml,' + encodeURIComponent(ICON);

const manifest = {
  name: 'Hütchenjagd', short_name: 'Hütchen', start_url: '.', display: 'standalone',
  orientation: 'portrait', background_color: '#16281f', theme_color: '#16281f',
  description: 'Hütchenjagd — einmal ums Brett und wieder heim, wenn dich keiner fängt. Komplett offline.',
...
<meta name="description" content="Hütchenjagd – Laufspiel mit Fangen für zwei. An einem Handy oder auf zweien, komplett offline.">
<title>Hütchenjagd</title>
...
if (webFlagge !== -1) {
  ...
  const mitnahme = 'Huetchenjagd.html';
...
if (fragmentFlag !== -1) {
  const out = resolve(process.argv[fragmentFlag + 1] || 'hut-fragment.html');
  writeFileSync(out, `<title>Hütchenjagd</title>\n<style id="hut-css">\n${css}\n</style>\n${body}\n`);
