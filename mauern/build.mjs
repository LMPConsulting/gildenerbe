// Baut aus mauern/src/* eine einzige, in sich geschlossene HTML-Datei.
// Kein CDN, keine externen Dateien: läuft per Doppelklick (file://) und offline.
//
//   node mauern/build.mjs            -> mauern/index.html
//   node mauern/build.mjs --fragment out.html   (Rumpf ohne <html>/<head>, für Artifacts)
...
// Mauern kommt ohne eigene Brettdatei aus: die Geometrie ist ein schlichtes
// Gitter und steht dort, wo sie gebraucht wird — Regeln in der Engine, Maße in
// der Oberfläche.
...
// Ein Gitter mit einer Mauer quer darin, zwei Figuren davor und dahinter.
const ICON = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">`
  + `<rect width="64" height="64" rx="14" fill="#10262b"/>`
  + `<rect x="8" y="8" width="48" height="48" rx="10" fill="#f0ece0"/>`
  + `<g fill="none" stroke="#d6cfbd" stroke-width="2.5">`
  + `<path d="M24 10v44M40 10v44M10 24h44M10 40h44"/></g>`
  + `<rect x="10" y="30.5" width="30" height="4.5" rx="1.5" fill="#a9743c"/>`
  + `<circle cx="16" cy="16" r="5" fill="#e0603f"/>`
  + `<circle cx="48" cy="48" r="5" fill="#3e93c4"/></svg>`;
const iconUrl = 'data:image/svg+xml,' + encodeURIComponent(ICON);

const manifest = {
  name: 'Mauern', short_name: 'Mauern', start_url: '.', display: 'standalone',
  orientation: 'portrait', background_color: '#10262b', theme_color: '#10262b',
  description: 'Mauern — laufen oder eine Mauer bauen. Wer zuerst drüben ist, gewinnt. Komplett offline.',
...
<meta name="description" content="Mauern – laufen oder mauern, für zwei. An einem Handy oder auf zweien, komplett offline. Nach Quoridor.">
<title>Mauern</title>
...
if (fragmentFlag !== -1) {
  const out = resolve(process.argv[fragmentFlag + 1] || 'mauern-fragment.html');
  writeFileSync(out, `<title>Mauern</title>\n<style id="mauern-css">\n${css}\n</style>\n${body}\n`);
