// Baut aus aerger/src/* eine einzige, in sich geschlossene HTML-Datei.
// Kein CDN, keine externen Dateien: läuft per Doppelklick (file://) und offline.
//
//   node aerger/build.mjs            -> aerger/index.html
//   node aerger/build.mjs --fragment out.html   (Rumpf ohne <html>/<head>, für Artifacts)
...
// Zwei Figuren und ein Würfel auf hellem Holz.
const ICON = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">`
  + `<rect width="64" height="64" rx="14" fill="#241c17"/>`
  + `<rect x="8" y="8" width="48" height="48" rx="10" fill="#efe2c8"/>`
  + `<circle cx="22" cy="22" r="7" fill="#c2453c"/>`
  + `<circle cx="42" cy="42" r="7" fill="#2f6fb2"/>`
  + `<circle cx="42" cy="22" r="2.6" fill="#2a201a"/>`
  + `<circle cx="22" cy="42" r="2.6" fill="#2a201a"/></svg>`;
const iconUrl = 'data:image/svg+xml,' + encodeURIComponent(ICON);

const manifest = {
  name: 'Ärger', short_name: 'Ärger', start_url: '.', display: 'standalone',
  orientation: 'portrait', background_color: '#241c17', theme_color: '#241c17',
  description: 'Ärger — vier Figuren, ein Würfel, kein Erbarmen. Komplett offline.',
...
<meta name="description" content="Ärger – der Würfelklassiker für zwei. An einem Handy oder auf zweien, komplett offline.">
<title>Ärger</title>
...
if (fragmentFlag !== -1) {
  const out = resolve(process.argv[fragmentFlag + 1] || 'aerger-fragment.html');
  writeFileSync(out, `<title>Ärger</title>\n<style id="aerger-css">\n${css}\n</style>\n${body}\n`);
