// Baut aus seeschlacht/src/* eine einzige, in sich geschlossene HTML-Datei.
// Kein CDN, keine externen Dateien: läuft per Doppelklick (file://) und offline.
//
//   node seeschlacht/build.mjs            -> seeschlacht/index.html
//   node seeschlacht/build.mjs --fragment out.html   (Rumpf ohne <html>/<head>, für Artifacts)
...
// Ein Raster mit zwei Treffern und einem Fehlschuss.
const ICON = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">`
  + `<rect width="64" height="64" rx="14" fill="#0c1f26"/>`
  + `<g fill="none" stroke="#24505e" stroke-width="2">`
  + `<rect x="10" y="10" width="44" height="44" rx="5"/>`
  + `<path d="M25 10v44M39 10v44M10 25h44M10 39h44"/></g>`
  + `<rect x="12.5" y="12.5" width="10" height="10" rx="2" fill="#e2603c"/>`
  + `<rect x="41.5" y="41.5" width="10" height="10" rx="2" fill="#e2603c"/>`
  + `<circle cx="32" cy="32" r="3" fill="#e8c98a"/></svg>`;
const iconUrl = 'data:image/svg+xml,' + encodeURIComponent(ICON);

const manifest = {
  name: 'Seeschlacht', short_name: 'Seeschlacht', start_url: '.', display: 'standalone',
  orientation: 'portrait', background_color: '#0c1f26', theme_color: '#0c1f26',
  description: 'Seeschlacht — zehn Schiffe verstecken, zehn Schiffe finden. Komplett offline.',
...
<meta name="description" content="Seeschlacht – Schiffe versenken für zwei. An einem Handy oder auf zweien, komplett offline.">
<title>Seeschlacht</title>
...
const body = `<div id="app"></div>\n<script id="seeschlacht-js">\n${script}\n</script>`;

const page = `<!doctype html>
<html lang="de">
<head>
${kopf}
<style id="seeschlacht-css">
${css}
</style>
</head>
<body>
...
if (webFlagge !== -1) {
  ...
  const mitnahme = 'Seeschlacht.html';
...
if (fragmentFlag !== -1) {
  const out = resolve(process.argv[fragmentFlag + 1] || 'seeschlacht-fragment.html');
  writeFileSync(out, `<title>Seeschlacht</title>\n<style id="seeschlacht-css">\n${css}\n</style>\n${body}\n`);
